...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        ...
        {
            super.onUpdate();
            EventMotion eventMotion = new EventMotion(this.rotationYaw,this.rotationPitch,this.posY,this.onGround);
            eventMotion.fire();
            if (eventMotion.isCancelled()) {
                eventMotion = new EventMotion ();
                eventMotion.fire();
                return;
            }
            if (eventMotion.isLockView()) {
                this.rotationYaw = eventMotion.getRotationYaw();
                this.rotationPitch = eventMotion.getRotationPitch();
                this.rotationYawHead = eventMotion.getRotationYaw();
            }
            if (this.isRiding())
            {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(eventMotion.getRotationYaw(), eventMotion.getRotationPitch(), this.onGround));
                this.sendQueue.addToSendQueue(new C0CPacketInput(this.moveStrafing, this.moveForward, this.movementInput.jump, this.movementInput.sneak));
            }
            else
            {
                this.onPlayerUpdate(eventMotion.getRotationYaw(),eventMotion.getRotationPitch(),eventMotion.getY(),eventMotion.isGround(),eventMotion.isSend());
            }
            eventMotion = new EventMotion();
            eventMotion.fire();
        }
    ...
    public void onPlayerUpdate(float eventYaw,float eventPitch,double eventY,boolean eventGround,boolean eventSend) {
        boolean var1 = this.isSprinting();
        if (var1 != this.field_175171_bO) {
            if (var1 && (!Skuxx.getInstance().getModManager().getMod("Jesus").getState() || !(Skuxx.getInstance().getModManager().getMod("Jesus").isRunning()))) {
        ...
        if (this.func_175160_A()) {
            ...
            double var5 = eventY - this.field_175166_bJ;
            double var7 = this.posZ - this.field_175167_bK;
            double var9 = (eventYaw - this.field_175164_bL);
            double var11 = (eventPitch - this.field_175165_bM);
            boolean var13 = var3 * var3 + var5 * var5 + var7 * var7 > 9.0E-4D || this.field_175168_bP >= 20;
            boolean var14 = var9 != 0.0D || var11 != 0.0D;

            if (this.ridingEntity == null) {
                if ((var13 && var14) || eventSend) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, eventY, this.posZ, eventYaw, eventPitch, eventGround));
                } else if (var13) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, eventY, this.posZ, eventGround));
                } else if (var14) {
            ...
            if (var13) {
                ...
                this.field_175166_bJ = eventY;
                this.field_175167_bK = this.posZ;
                this.field_175168_bP = 0;
            }

            if (var14) {
                this.field_175164_bL = eventYaw;
                this.field_175165_bM = eventPitch;
            }
        }
    ...
    {
        EventPlayerMove event = new EventPlayerMove(x, y, z);
        event.fire();
    ...
    {
        if (Skuxx.getInstance().getModManager().getMod("Aura").getState()) {
            Skuxx.getInstance().getModManager().getMod("Aura").toggle();
            NotificationManager.addNotification(new Notification("Aura disabled",false,40   ));
    ...
    protected void damageEntity(DamageSource p_70665_1_, float p_70665_2_)
    {
        if (!this.func_180431_b(p_70665_1_))
        {
            ...
            DamageTakeEvent dmgEvent = new DamageTakeEvent(p_70665_2_);
            dmgEvent.fire();
    ...
    {
        EventPush eventPush = new EventPush(!this.noClip);
        eventPush.fire();
    ...
    {
        super.setSprintingXD(sprinting);
    ...
    public void onLivingUpdate()
    {
        ...
        SlowlessEvent event = new SlowlessEvent();
        event.fire();
